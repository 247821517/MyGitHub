...
public class SeaboardMeshCreator
{
    ...
    public void CreateSeaboardMeshs(string savingPath)
    {
        for (int boardIndex = 0; boardIndex < listOfFrontVertexs.Count; boardIndex++)
        {
            ...
            Mesh newMesh = AssetDatabase.LoadAssetAtPath<Mesh>(savingPath + $"/seaboardMesh.mesh");
            bool alreadyExist = newMesh != null;
            if (!alreadyExist)
            {
                newMesh = new Mesh();
            }
            ...
            {
                AssetDatabase.CreateAsset(newMesh, savingPath + $"/seaboardMesh_{boardIndex}.mesh");
            }
...
public class SeaboardMeshCreateWindow : EditorWindow
{
    ...
    private void DrawMeshParams()
    {
        ...
        GUILayout.Space(20);
        bool needRepaint = false;
        float floatValue = EditorGUILayout.Slider("宽度", creator.meshWidth, 0, 5);
        ...
        floatValue = EditorGUILayout.Slider("偏移", creator.widthOffset, -5, 5);
